...
Клонировать другой репозиторий можно по ссылке на GitHub. Перейдем в каталог, куда надо клонировать, дальше команда
...
```
git branch [название ветки]my-new-branch
```

git branch - без указания создания ветки - выведет все ветки. Текущая будет отмечена звездочкой *

git checkout - переключения веток

```
git checkout master
git checkout feature/new-additions
```
...
Синтаксис: git diff [ветка1] [ветка2]

```
git diff master feature/diff
```

Можно вызвать лог по веткам: покажет все ветки и последние операции (коммиты с хешами):

```
git log --oneline
```

Можно выполнить сравнение ветки с хешем коммита из другой ветки (результат будет аналогичный):

```
$ git diff master b320803
# вывод будет такой же, как при использовании git diff master feature/diff 
```
...
Например, в проекте есть 2 ветки, одна из которых master, другая - feature/diff
Чтобы сделать слияние, переходим в проект и переключаемся на главную ветку (checkout master)
Устанавливаем слияние с feature/diff
...
Более безопасно: маленькая -d (удалит, только если было полное объединение с другой веткой).